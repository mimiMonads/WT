...
export default function Board () {
  ...
  const loadMessages = async (q = '') => {
    try {
      const url = q ? `${host}/api/messages/search/${encodeURIComponent(q)}` : `${host}/api/messages`
      ...
      <ul className="message-list">
        {messages.map((m) => (
          <li key={m._id} className="message-item">
            <p>{m.content}</p>
            <span className="meta">{new Date(m.createdAt || m.date || Date.now()).toLocaleString()}</span>
          </li>
        ))}
